...
class hwPricer:
    ...
    def zero_bond_put(self, option_expiry, bond_maturity, k, mc=False):
        ...
        if mc: # this is monte carlo simulation btw 
    ...
    def caplet(self, fixing_time, payment_time, N, k, method='js'):
        ...
        return N * caplet_value

    def cap(self, payment_schedule, N, k, mc=False):
        cap_value = 0
        if mc:
    ...
    def floor(self, payment_schedule, N, k, mc=False):
        floor_value = 0
        if mc:
            for i in range(1, len(payment_schedule)):
                fixing_time = payment_schedule[i - 1]
        ...
        return N * floor_value

    def swap(self, payment_schedule, N, fixed_rate, payer=True, mc=False):
        ...
        swap_value = N * direction * (floating_leg_pv - fixed_leg_pv)
        return swap_value

    def swaption(self, payment_schedule, N, fixed_rate, payer=True, mc=False):
        direction = 1 if payer else -1
        ...
        if mc:
            ...
            swaption_value = disc_expiry * N * np.mean(np.maximum(direction * (floating_leg_value - fixed_leg_value), 0))
            ...
            swaption_value = N * (floating_leg_value + fixed_leg_value)

        return swaption_value

    def coupon_bond(self, payment_schedule, coupon_rate, N):
        bond_price = 0
        for i in range(len(payment_schedule)):
            ...
            coupon_cashflow = N * coupon_rate * accrual_period
    ...
    def floating_rate_note(self, payment_schedule, N):
        disced_coupons = self.swap(payment_schedule, N, fixed_rate=0, payer=False, mc=False)
        disced_notional = N * self.model.discount(payment_schedule[-1])
        frn_price = disced_coupons + disced_notional
        return frn_price

    def bond_option(self, option_expiry, payment_schedule, coupon_rate, k, N, call=True, mc=False):
        if mc:
            bond_price_dist = self.curve_sim.coupon_bond(option_expiry, payment_schedule, coupon_rate, N)
            ...
            critical_rate = self.find_rstar_bond(option_expiry, payment_schedule, coupon_rate, N, k)
            option_value = 0
    ...
    def jams_root_bond(self, option_expiry, payment_schedule, coupon_rate, N, k_price, critical_rate):
        bond_price = 0
    ...
    def find_rstar_bond(self, option_expiry, payment_schedule, coupon_rate, N, k_price, lower_bound=-5.0, upper_bound=5.0):
        root_func = lambda rate: self.jams_root_bond(option_expiry, payment_schedule, coupon_rate, N, k_price, rate)
        try:
            critical_rate = brentq(root_func, lower_bound, upper_bound, xtol=1e-12)
        except ValueError:
            critical_rate = brentq(root_func, -10.0, 10.0, xtol=1e-12)
