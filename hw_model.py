...
class hwModel:
    ...
    def short_rate(self, maturity, z=None):
        ...
        mean_rate = (
            r0 * np.exp(-a * maturity)
            + self.theta(maturity)
            - np.exp(-a * maturity) * self.theta(0)
        )
...
class hwSim:
    ...
    def sim_short_rate_direct(self, future_time):
        ...
        simulated_rates = np.array(
            [self.model.short_rate(future_time, z=z_val) for z_val in shocks]
        )
    ...
    def sim_short_rate_direct_fwd(self, future_time):
        shocks = np.random.normal(size=self.n_paths)
        sim_rates = np.array(
            [self.model.short_rate_forward(future_time, z=z_val) for z_val in shocks]
        )
        return sim_rates
    ...
    def validate_sim(self, future_time):
        ...
        analytic_mean = (
            self.model.parameters['r0'] * np.exp(-self.model.parameters['a'] * future_time)
            + self.model.theta(future_time)
            - np.exp(-self.model.parameters['a'] * future_time) * self.model.theta(0)
        )
        analytic_std = np.sqrt(
            (self.model.parameters['sigma'] ** 2) / (2 * self.model.parameters['a'])
        ...
        df = pd.DataFrame(
            comparison_data,
            index=["Euler Simulation", "Direct Simulation", "Analytic"],
        )
        return df
...
class hwCurveBuilder:
    ...
    def zero_coupon_bond(self, eval_time, maturity, fwd_measure=False):
        ...
        A_factor = self.model.bond_adj_factor(eval_time, maturity)
        B_factor = self.model.rate_sens(eval_time, maturity)
    ...
    def long_rate(self, start_time, end_time, fwd_measure=False):
        ...
        A_factor = self.model.bond_adj_factor(start_time, end_time)
        B_factor = self.model.rate_sens(start_time, end_time)
